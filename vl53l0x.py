...
class VL53L0X:
	...
	def _registers(self, register, values=None, struct_type='B'):
		if values is None:
			size = struct.calcsize(struct_type)
			data = self.i2c.readfrom_mem(self.address, register, size)
			values = struct.unpack(struct_type, data)
			return values
		data = struct.pack(struct_type, *values)
		self.i2c.writeto_mem(self.address, register, data)

	def _register(self, register, value=None, struct_type='B'):
		if value is None:
			return self._registers(register, struct_type=struct_type)[0]
		self._registers(register, (value,), struct_type=struct_type)
	...
	def start(self, period=0):
		...
		if period:
			oscilator = self._register(_OSC_CALIBRATE, struct_type='>H')
			if oscilator:
				period *= oscilator
			self._register(_MEASURE_PERIOD, period, struct_type='>H')
	...
	def read(self):
		...
		for timeout in range(_IO_TIMEOUT):
			...
			raise TimeoutError()
		value = self._register(_RESULT_RANGE_STATUS + 10, struct_type='>H')
		self._register(_INTERRUPT_CLEAR, 0x01)
		return value
	...
	def perform_single_ref_calibration(self, vhv_init_byte):
		...
		while self._register((RESULT_INTERRUPT_STATUS & 0x07) == 0):
			if time.times_ms() - chrono > _IO_TIMEOUT:
				return False
