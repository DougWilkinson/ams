...
# reworked to include color palette and display_on and display_off

from versions import versions
versions[__name__] = 10
# 10: modified to work with my code and new with refactored code
...
class GC9A01(framebuf.FrameBuffer):
    ...
    def __init__(
        ...
        self.BLACK = self.create_color(0, 0, 0, 0)
        self.GREEN = self.create_color(1, 0, 255, 0)
        self.RED = self.create_color(2, 255, 0, 0)
        self.LIGHTRED = self.create_color(3, 140, 0, 0)
        self.BLUE = self.create_color(4, 0, 0, 255)
        self.YELLOW = self.create_color(5, 255, 255, 0)
        self.GREY = self.create_color(6, 100, 100, 100)
        self.MAGENTA = self.create_color(7, 255, 0, 255)
        self.CYAN = self.create_color(8, 0, 255, 255)
        self.LIGHTGREEN = self.create_color(9, 0, 100, 0)
        self.DARKGREEN = self.create_color(10, 0, 80, 0)
        self.DARKBLUE = self.create_color(11, 0, 0, 90)
        # 12, 13, 14 free for user definition
        self.WHITE = self.create_color(15, 255, 255, 255)


    def create_color(self, idx, r, g, b):
        c = GC9A01.rgb(r, g, b)
        if not 0 <= idx <= 15:
