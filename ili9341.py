...
class Ili9341(object):
	...
	def block(self, x0, y0, x1, y1, data):
		...
		"""
		if self.save_block:
			self.saved.append((x0,y0,x1,y1,data))
	...
	def poly(self, cx, cy, coords, color, fill):
		# print("cx={}, cy={}, coords={}".format(cx,cy,coords))
		x_values = []
		y_values = []
		for i in range(0,len(coords),2):
			x_values.append(coords[i])
		...
		for i in range(len(x_values) - 1):
			...
			x2 = x_values[i + 1]
			y2 = y_values[i + 1]
			self.draw_line(cx+x1, cy+y1, cx+x2, cy+y2, color)
		...
		for y in range(min_y, max_y + 1):
			...
			if len(intersect_x) % 2 != 0:
				intersect_x.pop()

			for i in range(0, len(intersect_x), 2):
				x1 = intersect_x[i]
				x2 = intersect_x[i + 1]
				self.draw_line(cx+x1, cy+y, cx+x2, cy+y, color)
	...
	def ellipse(self, x0, y0, a, b, color, fill=None):
		
		if fill:
			self.fill_ellipse(x0, y0, a, b, fill)
			return
		"""Draw an ellipse.
