...
class LEDMotionLight:
	def __init__(self, name, motion_pin, led_pin, num_leds=3, on_secs=300) -> None:
		self.state_name = name + "/set"
		self.bri_name = name + ""
		self.num_leds = num_leds
		#self.inbus_event = inbus_event
		debug("task create: {}".format(name))
		tasklist[name+"/set"] = asyncio.create_task(self.set_state(name+"/state"))
		tasklist[name+"_bri/set"] = asyncio.create_task(self.set_bri(name+"_bri/state"))
		tasklist[name+"_rgb/set"] = asyncio.create_task(self.set_rgb(name+"_rgb/state"))
